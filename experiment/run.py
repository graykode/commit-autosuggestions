...
def read_examples(filename):
    ...
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            nl = ' '.join(js['commit_tokens']).replace('\n', '')
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in tqdm(enumerate(examples), total=len(examples)):
        # source
        tokens = tokenizer([(example.added, example.deleted)],
                           max_length=args.max_source_length,
                           return_length=True, truncation=True)
        padding_length = args.max_source_length - tokens['length'][0]

        source_ids = tokens['input_ids'][0] + [tokenizer.pad_token_id] * padding_length
        source_mask = tokens['attention_mask'][0] + [0] * padding_length

        assert len(source_ids) == len(source_mask) == args.max_source_length

        # target
        if stage == "test":
            target_tokens = tokenizer.tokenize("None")
        else:
            verb = example.target.split(' ')[0]
        ...
        if example_index < 5:
            if stage == 'train':
                ...
                logger.info("source_tokens: {}".format(
                    [x.replace('\u0120', '_') for x in tokenizer.convert_ids_to_tokens(source_ids)]))
...
@slack_sender(webhook_url=webhook_url, channel="training-bot")
def main():
    ...
    parser.add_argument("--weight_random", action='store_true')
